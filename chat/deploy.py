...
if loading_type == "modelscope":
    ...
    os.system(f"git clone {model_name_or_path}")
...
def generate_interactive(
    ...
    **kwargs,
    ...
    while True:
        ...
        if additional_eos_token_id is not None and output_token_ids and output_token_ids[-1] == additional_eos_token_id:
...
def combine_history(prompt):
    ...
    total_prompt_parts = []  # 初始化total_prompt_parts列表
    ...
    for message in messages:
        ...
        if message["role"] == "user":
            ...
            raise RuntimeError(f"Unsupported role type: {message['role']}")
        total_prompt_parts.append(cur_prompt)  # 将cur_prompt添加到列表中

    cur_query_prompt = "<|im_start|>user\n{}\n<|im_end|>"  # 假设cur_query_prompt是这样定义的，或者它应该是某个已定义的变量
    total_prompt_parts.append(cur_query_prompt.format(user=prompt))  # 添加当前查询的提示
    total_prompt = ''.join(total_prompt_parts)  # 拼接所有部分形成完整的提示
    return total_prompt


def post_process_abc(output: str):
    """
    从ABC格式的文本中提取并返回ABC乐谱部分。

    如果文本中没有找到以"x:"开头的元数据行，则返回None。
    """
    ...
    #    return "\n".join(data)
    abc_notation = [splitted[metadata_idx].split(" ")[0]] + splitted[abc_idx:]
    return "\n".join(abc_notation)
...
def gen_wav(text):
    ...
    if abc_notation:
        ...
        # Convert MIDI to WAV
        tmp_wav = f"{tmp_path}/{launch_time}/{abc_time}.wav"
...
def main():
    ...
    if prompt := st.chat_input("Hey, melody time is coming!"):
        ...
        with st.chat_message("robot", avatar=robot_avator):
            ...
            # 使用最后一个完整的响应
            cur_response = responses[-1]
            message_placeholder.markdown(cur_response)

            # 假设chat_melody函数能够返回wav文件的路径
